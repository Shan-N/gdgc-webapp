...
import { CreditCard, CommandIcon } from 'lucide-react';

const RFIDCard = () => {
  return (
    <div className="flex justify-center items-center p-4">
      <motion.div 
        className="relative bg-gradient-to-br from-purple-600 to-indigo-700 text-white rounded-3xl shadow-xl overflow-hidden max-w-4xl w-full"
      ...
      >
        <div className="flex flex-col md:flex-row items-center p-8">
          <div className="flex-1 z-10">
            ...
            >
              Coming soon: Seamless access at our booth!
            </motion.p>
            <motion.button 
              className="bg-white text-purple-600 font-bold py-2 px-6 rounded-full hover:bg-opacity-90 transition duration-300"
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
            >
              <CommandIcon className="inline-block mr-2" />
              Get Notified
            </motion.button>
          </div>
          <motion.div 
          ...
          >
            <div className="relative w-64 h-64">
              ...
              <div className="absolute inset-4 bg-indigo-300 rounded-full opacity-40 animate-pulse"></div>
              <div className="absolute inset-8 bg-white rounded-full flex items-center justify-center">
                <CreditCard size={64} className="text-purple-600 rotate-45" />
              </div>
            </div>
          </motion.div>
        </div>
